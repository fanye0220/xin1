...
export async function syncCharacterToAndroid(char: CharacterCard, blobStoreValue: { avatarBlob?: Blob, originalFile?: File, avatarHistory?: Blob[] } | null, _allCharsCache?: CharacterCard[], _allChatsCache?: Map<string, any[]>): Promise<string[]> {
  if (!isAndroid()) return [];
